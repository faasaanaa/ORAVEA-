...
export default function WhatsAppFloating() {
	...
	// prefer plural env var VITE_WHATSAPP_NUMBERS, fallback to VITE_WHATSAPP_NUMBER
	const numbers = firstOrFallback('VITE_WHATSAPP_NUMBERS', 'VITE_WHATSAPP_NUMBER')
	const number = numbers.length ? numbers[0] : ''
			...
			<AnimatePresence>
				{open && (
					<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="mb-3 w-80 bg-brand-black-2 p-3 rounded-lg shadow-lg border border-black/30">
						...
						{numbers.length > 1 && (
							<div className="text-xs text-gray-400 mt-2">Using number: {number} (multiple configured)</div>
						)}
					</motion.div>
				)}
			</AnimatePresence>
